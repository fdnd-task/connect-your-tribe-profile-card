...
Na de installatie is de map `node_modules` aangemaakt, en gevuld met allerlei _packages_. Scroll eens door deze map heen; vele honderden *open source* ontwikkelaars hebben de packages die je ziet gebouwd en die mag je gratis gebruiken.

Start het voorbeeld visitekaartje op door in de terminal het commando `npm start` uit te voeren. Als het goed is, komt hier een melding te staan over het opstarten van de server: Application started on http://localhost:8000 — Open deze URL in je browser.
...
- [Nodejs.org](https://nodejs.org/en/), voor de installatie van NodeJS op jouw systeem, kies voor NodeJS 24.13.0 (LTS, Long Term Support). Dit is de meest stabiele versie van NodeJS, welke ondersteund wordt met goede documentatie.
- [How To Open Terminal](https://www.youtube.com/watch?v=OmQhOnBzg_k), om iemand de terminal te zien openen en gebruiken op Youtube.
...
Kopieer de CSS bestanden en alle afbeeldingen van jouw visitekaartje uit Sprint 1 naar de `/public` map. Bestanden in de `/public` map kunnen door de browser geladen worden; dit zijn statische bestanden, net als in Semester 1. Als je straks vanuit jouw HTML verwijst naar bestanden (via een `src` of `href` attribuut), dan doe je dat dus naar bestanden in de `/public` map. Je gebruikt `/public` alleen _niet_ in je eigen code, alsof die eigenlijk niet bestaat. Verwijs in je HTML bijvoorbeeld dus naar `/styles.css` of `/scripts/main.js`, en niet naar `/public/styles.css` en `/public/scripts/main.js`. Tip: Gebruik vanaf nu altijd een `/` voor je links (dus `href="/styles.css"` of `src="/assets/photo.jpg"`), zodat je altijd in de `/public` map uitkomt. Dit werkt net even anders dan op GitHub Pages, waar je met relatieve paden moest werken (`href="styles.css"` of `href="../styles.css"`).
...
Zet de rest van de HTML van jouw visitekaartje in `/views/index.liquid`. Let op: overschrijf bij het kopiëren de eerste en laatste regel niet! De code die je hier ziet, is de Liquid manier om bestanden te *renderen*: `{% render 'partials/head.liquid' %}` betekent “pak de inhoud van het `partials/head.liquid` bestand en plaats dat hier”. Dit opknippen van je code kon met alleen HTML niet.
